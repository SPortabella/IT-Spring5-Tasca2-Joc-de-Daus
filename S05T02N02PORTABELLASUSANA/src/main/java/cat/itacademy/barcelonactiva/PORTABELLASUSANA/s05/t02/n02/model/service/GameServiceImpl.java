...
import cat.itacademy.barcelonactiva.PORTABELLASUSANA.s05.t02.n02.model.repository.GameRepository;
...
public class GameServiceImpl implements GameService {
    ...
    @Override
    public List<Object> getAllGamesByIdPlayer(String id) {
    ...
    private List<RankingDTO> calcRanking() {
        ...
        for (Player player : players) {
            ...
            int countGameWin = (int) games.stream()
                    .filter(x -> x.getPlayer().getId().equals(id) && x.getPoints() == 7)
                    .count();
            int totalGames = (int) games.stream()
                    .filter(x -> x.getPlayer().getId().equals(id))
                    .count();
